...
```{r}
# Manual AIC and BIC calculations
...
#### 4.1 New variable creation: unions
...
```{r}
# make long dataset: df_long
df_long <- pivot_longer(dataset, 
                        cols = -c(Country, Year),  
                        names_to = "Variable",
                        values_to = "Value")
```
...
#### 4.2 K-means clustering and plotting
...
#### 4.3 Plot the average values of demo, civsoc_particip, and rol over time
...
correlation_matrix <- cor(dataset[, c("demo", "unions","CollectiveBargain_Coverage", "Inflation", "WageGrowth", "Gov_debt", "ln_labor", "WomenUnemployment_rate", "net_export", "net_FDI", "Tax_contribution")])
