...
# Fit a linear model
linear_model <- plm(Gini_Index ~ TradeUnions_Density, data = pdata, model = "within")
# Summarize the models
...
non_linear_model <- plm(Gini_Index ~ TradeUnions_Density + I(TradeUnions_Density^2), data = pdata, model = "within")
...
aic_unions <- AIC(model_unions)
aic_u1 <- AIC(model_u1)
aic_u2 <- AIC(model_u2)
aic_u3 <- AIC(model_u3)

bic_unions <- BIC(model_unions)
bic_u1 <- BIC(model_u1)
bic_u2 <- BIC(model_u2)
bic_u3 <- BIC(model_u3)

# Compare AIC and BIC values
aic_values <- c(aic_unions, aic_u1, aic_u2, aic_u3)
bic_values <- c(bic_unions, bic_u1, bic_u2, bic_u3)
model_names <- c("model_unions", "model_u1", "model_u2", "model_u3")
...
| \<chr\>      | \<dbl\>  | \<dbl\>  |          |          |
+:=============+=========:+=========:+==========+==========+
| model_unions | 3858.953 | 3872.716 |          |          |
+--------------+----------+----------+----------+----------+
| model_u1     | 4105.271 | 4119.034 |          |          |
+--------------+----------+----------+----------+----------+
| model_u2     | 3847.915 | 3861.677 |          |          |
+--------------+----------+----------+----------+----------+
| model_u3     | 4075.807 | 4089.570 |          |          |
+--------------+----------+----------+----------+----------+

```         
The best model according to AIC is: model_u2 
The best model according to BIC is: model_u2 
```
...
```{r}
model_formulas <- c(
  "Gini_Index ~ density2 + Inflation + WageGrowth + lnminwage",
  
  "Gini_Index ~ density2 + Inflation + WageGrowth + lnminwage + CollectiveBargain_Coverage",
  
  "Gini_Index ~ density2 + Inflation + WageGrowth + lnminwage + CollectiveBargain_Coverage + lnGDP + Gov_debt",
...
# Relevance: Check if demo is statistically significant
summary(first_stage)

# Exogeneity: Check correlation between demo and residuals
...
second_stage <- lm(Gini_Index ~ demo + CollectiveBargain_Coverage + Inflation + WageGrowth + Gov_debt + ln_labor + WomenUnemployment_rate + net_export, data = dataset)

# Exclusion Restriction: Check if demo is not statistically significant in the second-stage regression
summary(second_stage)
```

### **6.1.2 Detailed Explanations:**

1.  **First-Stage Regression**:

    -   This step regresses the endogenous variable (**`density2`**) on the instrument (**`demo`**) and other control variables.

    -   The goal is to check if **`demo`** is a strong predictor of **`density2`**, thus validating its relevance.

2.  **Relevance**:

    -   In the first-stage regression, check the coefficient of **`demo`** to see if it is statistically significant (typically p-value \< 0.05).

    -   A significant coefficient indicates that **`demo`** is relevant.

3.  **Exogeneity**:

    -   Residuals from the first-stage regression are computed.

    -   The correlation between **`demo`** and these residuals is checked. A correlation close to zero supports the assumption that **`demo`** is exogenous (not correlated with the error term).

    -    good instrumental variable is highly correlated with one or more of the explanatory variables while remaining uncorrelated with the errors.

4.  **Second-Stage Regression**:

    -   This step regresses the dependent variable (**`Gini_Index`**) on the predicted value of **`density2`** (obtained from the first stage) and other control variables.

    -   **`demo`** is included in the model to test its exclusion restriction.

5.  **Exclusion Restriction**:

    -   By summarizing the second-stage regression, we check if **`demo`** is statistically significant in this model.

    -   If **`demo`** is not significant, it supports the exclusion restriction assumption, meaning **`demo`** affects the **`Gini_Index`** only through **`density2`**.

This commented code provides a clear framework for validating the instrumental variable and performing a two-stage least squares (2SLS) regression analysis to address endogeneity concerns in your econometric model.
...
```{r}
library(AER)
model4_formula <- ivreg(Gini_Index ~ density2 + CollectiveBargain_Coverage + Inflation + WageGrowth + lnminwage + lnGDP + Gov_debt + ln_labor + Female_Workers + Parttime_employment + WomenUnemployment_rate | TradeUnions_Density + demo + Inflation + WageGrowth + lnminwage + CollectiveBargain_Coverage + lnGDP + Gov_debt + ln_labor + Female_Workers + Parttime_employment + WomenUnemployment_rate, data = dataset)

model5_formula <- ivreg(Gini_Index ~ density2 + CollectiveBargain_Coverage + Inflation + WageGrowth + lnminwage + lnGDP + Gov_debt + ln_labor + Female_Workers + Parttime_employment + WomenUnemployment_rate + net_export | TradeUnions_Density + demo + Inflation + WageGrowth + lnminwage + CollectiveBargain_Coverage + lnGDP + Gov_debt + ln_labor + Female_Workers + Parttime_employment + WomenUnemployment_rate + net_export, data = dataset)

model6_formula <- ivreg(Gini_Index ~ density2 + CollectiveBargain_Coverage + Inflation + WageGrowth + lnminwage + lnGDP + Gov_debt + ln_labor + Female_Workers + Parttime_employment + WomenUnemployment_rate + net_export + net_FDI + Tax_contribution | TradeUnions_Density + demo + Inflation + WageGrowth + lnminwage + CollectiveBargain_Coverage + lnGDP + Gov_debt + ln_labor + Female_Workers + Parttime_employment + WomenUnemployment_rate + net_export + net_FDI + Tax_contribution, data = dataset)

# Estimate the models
model4 <- summary(model4_formula)
model5 <- summary(model5_formula)
model6 <- summary(model6_formula)
summary(model6_formula)
```
