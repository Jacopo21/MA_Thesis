...
model_u1 <- lm(Gini_Index ~ u1, data = dataset)
model_u2 <- lm(Gini_Index ~ u2, data = dataset)
model_u3 <- lm(Gini_Index ~ u3, data = dataset)
...
correlation_matrix <- cor(dataset[, c("demo", "density2", "Inflation", "WageGrowth", "Gov_debt", "ln_labor", "WomenUnemployment_rate", "net_export", "net_FDI", "Tax_contribution")])
...
vif_values <- car::vif(lm(Gini_Index ~ demo + density2 + Inflation + WageGrowth + Gov_debt + ln_labor + WomenUnemployment_rate + net_export + net_FDI + Tax_contribution, data = dataset))
```
